...
if [ $nuttcp -eq 1 ]; then
    ...
    for buffer in "${buffer_size[@]}"; do # buffer
        ...
        for i in $(seq 1 $TESTS); do # TCP 50 runs
            ...
            echo "RUN: $i ($buffer KB) - Partial transfer: $part_transfer_tcp $units_transfer_tcp - Partial throughput: $part_bitrate_tcp $units_bitrate_tcp - Partial retransmissions: $part_retransmissions_tcp - Partial RTT: $part_latency_tcp $units_latency_tcp"
        done # TCP 50 runs
